...
export async function getTrimmedAccessTree(accessTree) {
    return new Promise(function (resolve, reject) {
            ...
            accessTree.country.states.map((state, stateIndex) => {
                if (state.selected == true) {
                    ...
                    if (state.districts != undefined)
                        state.districts.map((district, districtIndex) => {
                            if (district.selected == true) {
                                trimmedAccessTree.country.states[stateIndex].districts[districtIndex].recursive = 1;
                                trimmedAccessTree.country.states[stateIndex].districts[districtIndex].cities = undefined;
                                selectedDistrictCount++;
                                activeDistrictCount++;
                            } else {
                                trimmedAccessTree.country.states[stateIndex].districts[districtIndex].recursive = 0;
                                ...
                                if (district.cities != undefined)
                                    district.cities.map((city, cityIndex) => {
                                        if (city.selected == true) {
                                            trimmedAccessTree.country.states[stateIndex].districts[districtIndex].cities[cityIndex].recursive = 1;
                                            trimmedAccessTree.country.states[stateIndex].districts[districtIndex].cities[cityIndex].complexes = undefined;
                                            selectedCitiesCount++;
                                            activeCitiesCount++;
                                        } else {
                                            trimmedAccessTree.country.states[stateIndex].districts[districtIndex].cities[cityIndex].recursive = 0;
                                            ...
                                            if (city.complexes != undefined)
                                                city.complexes.map((complex, complexIndex) => {
                                                    if (complex.selected == true) {
                                                        ...
                                                        if (trimmedAccessTree.country.states[stateIndex] != undefined
                                                            && trimmedAccessTree.country.states[stateIndex].districts[districtIndex] != undefined
                                                            && trimmedAccessTree.country.states[stateIndex].districts[districtIndex].cities[cityIndex] != undefined) {
                                                            //Remove Complex
                                                            //trimmedAccessTree[stateIndex].districts[districtIndex].cities[cityIndex].complexes.splice(complexIndex,1);
                                                            delete trimmedAccessTree.country.states[stateIndex].districts[districtIndex].cities[cityIndex].complexes[complexIndex];
                                            ...
                                            if (selectedComplexesCount == 0) {
                                                if (trimmedAccessTree.country.states[stateIndex] != undefined
                                                    ...
                                                    delete trimmedAccessTree.country.states[stateIndex].districts[districtIndex].cities[cityIndex];
                                ...
                                if (activeCitiesCount == 0) {
                                    if (trimmedAccessTree.country.states[stateIndex] != undefined) {
                                        //Remove District
                                        delete trimmedAccessTree.country.states[stateIndex].districts[districtIndex];
        ...
        for (var stateIndex = 0; stateIndex < trimmedAccessTree.country.states.length; stateIndex++) {
            state = trimmedAccessTree.country.states[stateIndex];

            var district;
            var popDistrictCount = 0;
            if (state != undefined && state.districts != undefined) {
                for (var districtIndex = 0; districtIndex < state.districts.length; districtIndex++) {
                    district = state.districts[districtIndex];

                    var city;
                    var popCityCount = 0;
                    if (district != undefined && district.cities != undefined) {
                        for (var cityIndex = 0; cityIndex < state.districts.length; cityIndex++) {
                            city = district.cities[cityIndex];

                            var complex;
                            var popComplexCount = 0;
                            if (city != undefined && city.complexes != undefined) {
                                for (var complexIndex = 0; complexIndex < city.complexes.length; complexIndex++) {
                                    complex = city.complexes[complexIndex];
                                    if (complex == undefined) {
                                        resultantTree.country.states[stateIndex-popStateCount]
                                        .districts[districtIndex-popDistrictCount]
                                        .cities[cityIndex-popCityCount].complexes.splice(complexIndex - popComplexCount, 1);
                                ...
                                if(city == undefined){
                                    resultantTree.country.states[stateIndex-popStateCount]
                                    .districts[districtIndex-popDistrictCount].cities.splice(cityIndex - popCityCount, 1);
                                    popCityCount++;
                                }
                            }
                        }
                        ...
                        if(district == undefined){
                            resultantTree.country.states[stateIndex-popStateCount].districts.splice(districtIndex - popDistrictCount, 1);
                            popDistrictCount++;
                        }
                    }
                }
