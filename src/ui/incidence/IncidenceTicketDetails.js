...
import {
  ...
  Input,
  InputGroup,
  Modal, ModalHeader, ModalBody, ModalFooter, InputGroupText, InputGroupAddon, Form
} from "reactstrap";
...
import { getListingS3, executeTicketActionLambda, executeProgressLambda, executeFetchTeamListLambda } from "../../awsClients/incidenceLambdas";
...
function IncidenceTicketDetails(props) {
  ...
  const getClientSideSeniority = (user) => {
    ...
    if (superAdmin === user.userRole)
      return 4
    if (ClientSuperAdmin === user.userRole)
      return 3
    if (ClientAdmin === user.userRole)
      return 2
    if (ClientManager === user.userRole)
      return 1
    else
      return 0
  }
  ...
  async function executeGetImage() {
    let fileName = await getListingS3(complex)
    setImage(fileName)
  }
  useEffect(() => {
    executeGetImage()
  ...
  async function initCreateTicketRequest(createUserRequest) {
    ...
    try {
      ...
      await executeTicketActionLambda(requestCopy)
      this.messageDialog.current.showDialog("Progress Submitted", "Ticket Progress successfully submitted");
      loadingDialog.current.closeDialog();
    } catch (err) {
      loadingDialog.current.closeDialog();
      messageDialog.current.showDialog("Progress Submitted", "Ticket Progress successfully submitted", () => { props.history.goBack() });
  ...
  async function initProgressRequest() {
    try {
      let result = await executeProgressLambda(complex)
      let teamList = await executeFetchTeamListLambda(data)
  ...
  let AcceptDetails = {
    ...
    actionCode: ticketAction === "Accept Ticket" ? 1 :
      ticketAction === "Assign Ticket" ?
        2 : ticketAction === "Submit Progress" ?
          3 : ticketAction === "Mark Resolved" ?
            4 : ticketAction === "Mark Closed" ?
              5 : ticketAction === "Re-Open Ticket" ?
                6 : "",
    comment: "",
    assigneeId: data.lead_id ? data.lead_id : (!data.lead_id && id && !userSelect) ? id : (!data.lead_id && id && userSelect) ? userSelect[0] : "",
    assigneeRole: data.lead_role ? data.lead_role : (!data.lead_role && role && !userSelect) ? role : (!data.lead_role && role && userSelect) ? userSelect[1] : ""
  }
  ...
  console.log(id === data.lead_id && user.userRole === role, "id === data.lead_id && user.userRole === role");
                  ...
                  <div style={{ ...dashboardStyle.itemDescriprtion }}>
                    <div>
                      <div
                        ...
                        <p style={{
                          ...dashboardStyle.titleblack
                        }} className="text-muted">Summary</p>
                        {
                          status === "Closed" ? (
                            null
                            ...
                            <>
                              ...
                              >
                                ...
                                {status === "Queued" ? (
                                  <>
                                    <option
                                      value="Assign Ticket">Assign Ticket
                                    </option>
                                    <option
                                      value="Accept Ticket">Accept Ticket
                                    </option>
                                  </>
                                ) : (status === "Assigned" || status === "Open") ? (
                                  <>
                                    <option
                                      value="Mark Resolved">Mark Resolved
                                    </option>
                                    <option
                                      value="Submit Progress">Submit Progress
                                    </option>
                                  </>
                                ) : status === "Resolved" && (
                                  <>
                                    <option
                                      value="Mark Closed">Mark Closed
                                    </option>
                                  </>
                                )}
                              </Input>
          ...
          <TabPane tabId="2">
            <Row>
              <Col sm="12">
                ...
                {displayData && displayData !== undefined && displayData.length !== 0 &&
                            ...
                            <div
                              ...
                              <div>
                                <p style={{
                                  margin: "auto",
                                  ...dashboardStyle.titleblack,
                                }} className="text-muted">Event:&ensp;
                                  <span style={{
                                    ...dashboardStyle.itemDescriprtionBold2,
                                    fontStyle: "normal"
                                  }}>
                                ...
                                <p style={{
                                  ...
                                  {item.current_status === "Raised" ? "Ticket was raise with malfunction details" :
                                    item.current_status === "Assigned" ? "A lead was assigned to the ticket" :
                                      item.current_status === "opened" ? "Ticket marked open. First activity recorded from the team" :
                                        item.current_status === "Resolved" ? "The issue was reported to be resolved by the team" :
                                          item.current_status === "Closed" && "The resolution was accepted and this ticket has been closed"
                                  }
                                </p>
                              </div>
                              <div>
                                <p style={{
                                  ...dashboardStyle.titleblack,
                                  margin: "auto"
                                }} className="text-muted">{moment(date2).fromNow()}</p>
                                <p style={{
                                  ...dashboardStyle.titleblack,
                                  margin: "auto"
                                }} className="text-muted">{item.user_role}</p>
                                <p style={{
                                  ...dashboardStyle.titleblack
                                  , margin: "auto"
                                }} className="text-muted">{item.user_id}</p>
                              </div>
                            </div>
                            <div>
                              <p style={{
        ...
        >
          {
            modalShow === "Assign Ticket" ? (
              <><ModalHeader style={{ background: '#5DC0A6', color: `white`, padding: "20px" }} toggle={popUp}>Assign Ticket</ModalHeader><ModalBody
                style={{
                  width: "100%",
                  height: '400px',
                }}
              >
                ...
                <div>
                  <p
                    ...
                    To assign the ticket to the selected member type 'assign lead' in confirm action slot.
                    You can also provide comments to update the creator.
                  </p>
                  <Form>
                    <InputGroup className="mb-4">
                      ...
                      >
                        ...
                        {teamData && teamData !== undefined && teamData.length !== 0 &&
                    ...
                    <InputGroup className="mb-4">
                      ...
                      <Input
                        ...
                        onChange={(event) => (AcceptDetails.comment = event.target.value)} />
                    </InputGroup>
                    ...
                    <InputGroup className="mb-3">
                      ...
                      <Input
                        ...
                        onChange={(event) => (AcceptDetails.title = event.target.value)} />
                    </InputGroup>
                  </Form>
                </div>
              </ModalBody></>
            ) : modalShow === "Accept Ticket" ? (
              <><ModalHeader style={{ background: '#5DC0A6', color: `white` }} toggle={popUp}>Accept Ticket</ModalHeader><ModalBody
                style={{
                  width: "100%",
                  height: '400px',
                }}
              >
                <div>
                  <p
                    ...
                    To accept 'Lead' role for htis ticket type 'accept lead' in confirm
                    action slot. You can also provide comments to update the creator
                  </p>
                  <Form>
                    <InputGroup className="mb-4">
                      <InputGroupAddon addonType="prepend">
              ...
              <><ModalHeader style={{ background: '#5DC0A6', color: `white` }} toggle={popUp}>Mark Resolved</ModalHeader><ModalBody
                style={{
                  width: "100%",
                  height: '400px',
                }}
              >
                <div>
                  <p
                    ...
                    To mark the ticket 'Resolved', type 'mark resolved' in confirm
                    action slot. You can also provide comments to update the creator
                  </p>
                  <Form>
                    <InputGroup className="mb-4">
              ...
              <><ModalHeader style={{ background: '#5DC0A6', color: `white` }} toggle={popUp}>Submit Progress</ModalHeader><ModalBody
                style={{
                  width: "100%",
                  height: '400px',
                }}
              ...
              <><ModalHeader style={{ background: '#5DC0A6', color: `white` }} toggle={popUp}>Mark Closed</ModalHeader><ModalBody
                style={{
                  width: "100%",
                  height: '400px',
                }}
              >
                <div>
                  <p
                    ...
                    To mark the ticket 'Closed', type 'close ticket' in confirm
                    action slot. You can also provide comments to update the creator
                  </p>
                  <Form>
                    <InputGroup className="mb-4">
